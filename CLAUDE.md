...
A public CEFR curriculum served by curriculum-service (tracker: `../../docs/wordsly-path/PROGRESS.md`). Routes live under `app/path/` (`/path` map, `/path/unit/[unitId]`, `/path/lesson/[lessonId]` player, `/path/checkpoint/[unitId]` unit test, `/path/placement` placement test, `/path/review`), components in `components/features/path/`, data in `apis/path.api.ts` + `queries/path.query.ts` (keys under `queryKeys.path`), response types in `types/path/path.type.ts` (copied from the backend; keep them in step), pure helpers in `lib/path/path-tree.ts`.
...
- Progress writes (`enroll`, `complete`, checkpoint submit) return the new `me`: write it into the cache, then invalidate the rest of `path` (units carry lock state). `complete`, checkpoint and placement submit also invalidate `learningReport` (Path counts and badges on `/progress`; the badges are unlocked over Kafka, so they can lag a moment).
- `/progress` shows a Wordsly Path card (`components/features/progress/path-progress-card.tsx`, only when the learner has Path activity), a Path line in the accuracy chart, and Path badges (`lessons`, `units`, `stages` in `CATEGORY_ORDER`). Report types live in `types/learning-report/learning-report.type.ts`; keep them in step with learning-service.
...
- Placement (`components/features/path/placement/`, helpers in `lib/path/placement.ts`, tested) works like the checkpoint (online only, one submit with the served `releaseId` and one `clientRequestId` per attempt, the query kept out of the submit's invalidation), plus: an intro screen, "I don't know" (answers `null`) and "stop here" (sends what was answered, the rest padded with `null` by `padPlacementAnswers`). The server grades, enrolls and moves the start unit forward only; answers are never shown, the result names the start unit and which probed units counted as known. `GET /path/placement` 404s when no test is published: the API returns `null` and the page offers the path instead. Entry points: the not-enrolled hero ("Find my level" beside "Start from the beginning") and, for a learner who enrolled straight in and hasn't done a lesson, a link under the hero.
...
- `/admin`: dashboard (`admin-dashboard.tsx`): the last 30 days as tiles (`StatTiles`, the generic half of `StatsCards`) from `GET /admin/users/stats` and `GET /admin/learning/stats`, two daily charts, and links into each area.
- `/admin/reports` (`components/features/admin-reports/`): a 7/30/90-day range and a tab (Growth, Learning, Wordsly Path, Content) in the URL (`lib/search-params/admin-reports.ts`); each tab fetches only its own stats. Data: `apis/admin-stats.api.ts`, `queries/admin-stats.query.ts` (`queryKeys.adminStats`; the hardest-items query asks learning-service for accuracy, then curriculum-service `items/lookup` for the text). Learning days are the learners' own calendars, account and Path days are UTC; the charts say so. Helpers in `lib/admin/stats.ts` (tested).
- Admin charts: `components/features/admin/charts` (`AdminBarChart`, `AdminPercentLineChart`, both `next/dynamic`), one series each in `--chart-1`, so no legend; a second measure gets its own chart, never a second axis. Retention is an HTML table shaded with `color-mix` on `--chart-1` (strong cells switch to `text-primary-foreground`), tables (`MiniTable`) back the rest.
...
- The command palette lists the admin pages (Dashboard, Reports, Users, Wordsly Path) for admins only.
